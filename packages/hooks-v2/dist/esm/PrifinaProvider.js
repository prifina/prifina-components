import { objectWithoutProperties as _objectWithoutProperties, objectSpread2 as _objectSpread2, slicedToArray as _slicedToArray, typeof as _typeof, defineProperty as _defineProperty } from './_virtual/_rollupPluginBabelHelpers.js';
import React, { createContext, useRef, useState, useCallback, useContext, useMemo } from 'react';
import { shortId, stringify } from './utils.js';
import gql from 'graphql-tag';
import { buildProviderFilter } from './queryBuilder.js';
...
var PrifinaContext = /*#__PURE__*/createContext({});
...
var PrifinaContextProvider = function PrifinaContextProvider(_ref) {
  ...
  var providerContext = useRef(_objectSpread2({
    stage: stage
  }, props));
  var callbacks = useRef({});
  var appSubscriptions = useRef({});
  var CLIENT = useRef({});
  var API = useRef({});

  var _useState = useState(activeUser || {
    name: "Test",
    uuid: "Testing-uuid",
  ...
  */
  ...
  var createSubscription = function createSubscription(opts) {
    ...
    if ((_callbacks$current = callbacks.current) !== null && _callbacks$current !== void 0 && _callbacks$current.sandbox) {
      ...
      callbacks.current["sandbox"][0](_defineProperty({}, connectorFunction.name, connectorFunction));
    }
    ...
    return new Promise(function (resolve, reject) {
      ...
      var _opts$variables = opts.variables,
          ...
          variables = _objectWithoutProperties(_opts$variables, _excluded2);
      ...
      if ((_appSubscriptions$cur = appSubscriptions.current) !== null && _appSubscriptions$cur !== void 0 && _appSubscriptions$cur[opts.appId]) {
        ...
        appSubscriptions.current[opts.appId] = _defineProperty({}, appHandler, subHandler);
      }
  ...
  var createMutation = function createMutation(opts) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      CLIENT.current.user.mutate({
        ...
        variables: {
          input: _objectSpread2(_objectSpread2({}, opts.variables), {}, {
            dataconnector: opts.name,
            userId: currentUser.uuid,
            appId: opts.appId,
            execId: shortId(),
        ...
        */
        ...
        if ((_callbacks$current5 = callbacks.current) !== null && _callbacks$current5 !== void 0 && _callbacks$current5.sandbox) {
          callbacks.current["sandbox"][0]({
            mutationResult: {
              data: _defineProperty({}, key, dataObject)
            }
          });
        }
  ...
  var createQuery = function createQuery(opts) {
    ...
    if ((_callbacks$current6 = callbacks.current) !== null && _callbacks$current6 !== void 0 && _callbacks$current6.sandbox) {
      ...
      var filter = opts !== null && opts !== void 0 && opts.filter ? stringify(opts.filter) : "";
      var connectorFunction = Object.assign({}, opts);
    ...
    if ((_callbacks$current8 = callbacks.current) !== null && _callbacks$current8 !== void 0 && _callbacks$current8.sandbox) {
      callbacks.current["sandbox"][0]({
        queryRequest: {
          appID: opts.appId,
          fields: queryFields,
          filter: opts !== null && opts !== void 0 && opts.filter ? buildProviderFilter(opts.filter) : "",
          next: opts.next
        }
      });
    }
  ...
  var registerDataConnector = useCallback(function (appID, modules) {
    if (modules.length > 0) {
      API.current[appID] = {};
      modules.forEach(function (module, mi) {
        ...
        console.log("LIST ", _typeof(functionList), functionList);
  ...
  var registerClient = useCallback(function (client) {
    CLIENT.current["user"] = client[0];
    CLIENT.current["prifina"] = client[1];
    CLIENT.current["s3"] = client[2];
  }, []);
  var onUpdate = useCallback(function (appID, fn) {
  ...
  var getAppSubscriptions = useCallback(function () {
    //console.log("GET CALLBACk ", data);
    return appSubscriptions.current;
  }, []);
  var getCallbacks = useCallback(function () {
    //console.log("GET CALLBACk ", data);
    return callbacks.current;
  }, []);
  var removeCallbacks = useCallback(function () {
    callbacks.current = {};
    return true;
  }, []);
  var deleteCallback = useCallback(function (appID, index) {
  ...
  var getAPI = useCallback(function () {
    return API.current;
  }, []);
  var getLocalization = useCallback(function () {
...
var usePrifina = function usePrifina() {
  ...
  var prifinaContext = useContext(PrifinaContext); // console.log("CONTEXT2 ",prifinaContext);

  var prifina = useMemo(function () {
...
export { PrifinaContext, PrifinaContextProvider, usePrifina };
